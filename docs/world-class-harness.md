...
| P2 | The development lock is Agno 3.0.6; 2.6.4 remains the legacy boundary and 2.9.0 remains a required stable-v2 lane after a release-by-release audit. | Hosted certification and future release monitoring remain required as upstream moves quickly. |
...
await run.command(Respond(pending[0].request_id, True))
...
- Turn the complete [Agno release-practice audit](agno-release-practices.md) into tested
  ...
  3.0.6 primary; generate future ranges from exact release-candidate evidence.
