...
class MediaToolkit(Toolkit):
    ...
    def read_image(self, path: str) -> str:
        ...
        try:
            ...
            return f"Image: {file_path.name} ({mime}, {size_kb:.1f} KB)\ndata:{mime};base64,{b64}"
    ...
    @staticmethod
    def _read_pdf_pymupdf(path: Path, page_indices: list[int] | None = None) -> str:
        ...
        indices: Iterable[int] = page_indices or range(total_pages)
        for i in indices:
            if 0 <= i < total_pages:
                page = doc[i]
    ...
    @staticmethod
    def _read_pdf_pypdf(path: Path, page_indices: list[int] | None = None) -> str:
